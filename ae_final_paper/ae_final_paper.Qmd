...
In our analysis, we utilized the R programming language to fit a logit method to the data. Specifically, we employed the logit function，polr, see @polr-appendix , which is suitable for estimating ordered choice model as our baseline.
...
Given that the proportional odds assumption does not hold, we need to explore alternative models. In this case, we opted for the ordered probit model by using clm algorithm, see @clm-appendix, which does not rely on the proportional odds assumption.
